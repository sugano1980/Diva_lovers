tyrano.plugin.kag.menu={tyrano:null,kag:null,snap:null,init:function(){},
...
var that=this;this.kag.stat.is_skip=false;
...
this.kag.html("menu",{"novel":$.novel},
function(html_str){
var j_menu=$(html_str);
...
e.stopPropagation()});
layer_menu.find(".menu_close").click(function(e){layer_menu.hide();
if(that.kag.stat.visible_menu_button==true)$(".button_menu").show();
e.stopPropagation()});
layer_menu.find(".menu_window_close").click(function(e){that.kag.layer.hideMessageLayers();
layer_menu.hide();
if(that.kag.stat.visible_menu_button==true)
$(".button_menu").show();e.stopPropagation()});
layer_menu.find(".menu_save").click(function(e){if(button_clicked==true)return;
button_clicked=true;
that.displaySave();
e.stopPropagation()});
layer_menu.find(".menu_load").click(function(e){if(button_clicked==true)return;
button_clicked=true;
tyrano.plugin.kag.variable.sf.hogehoge =  "./index_2.html";
that.kag.backRitle();
e.stopPropagation()});
layer_menu.find(".menu_back_title").click(function(){
that.kag.backTitle()});
$.preloadImgCallback(j_menu,function(){layer_menu.fadeIn(300);
$(".button_menu").hide()},that)})},


displaySave:function(cb){var that=this;this.kag.stat.is_skip=false;
var array_save=that.getSaveData();
var array=array_save.data;
var layer_menu=that.kag.layer.getMenuLayer();
for(var i=0;i<array.length;i++)array[i].num=i;
this.kag.html("save",{array_save:array,"novel":$.novel},
function(html_str){var j_save=$(html_str);j_save.find(".save_list").css("font-family",that.kag.config.userFace);j_save.find(".save_display_area").each(function(){$(this).click(function(e){var num=
...
$.getStorage(this.kag.config.projectID+"_tyrano_quick_save",this.kag.config.configSave);if(data)data=JSON.parse(data);else return false;this.loadGameData($.extend(true,{},data))},
doSetAutoSave:function(){
var data=this.snap;
data.save_date=$.getNowDate()+"\u3000"+$.getNowTime();$.setStorage(this.kag.config.projectID+"_tyrano_auto_save",data,this.kag.config.configSave)},
loadAutoSave:
function(){
...
this.loadGameData_2($.extend(true,{},data),{"auto_next":"yes"})},
...
if(flag_thumb=="false"){var img_code="";var data={};data.title=title;data.stat=_stat;data.current_order_index=_current_order_index;data.save_date=$.getNowDate()+
...
20)}},createImgCode:function(canvas){var code="";var q=this.kag.config.configThumbnailQuality;if(q=="low")code=canvas.toDataURL("image/jpeg",0.3);else if(q=="middle")code=canvas.toDataURL("image/jpeg",0.7);else code=canvas.toDataURL();return code},setGameSleep:function(next_flag){if(next_flag)this.kag.tmp.sleep_game_next=true;else this.kag.tmp.sleep_game_next=false;this.kag.tmp.sleep_game=this.snap},
...
var layer_menu=that.kag.layer.getMenuLayer();
for(var i=0;i<array.length;i++)array[i].num=i;
this.kag.html("load",{array_save:array,"novel":$.novel},
function(html_str){var j_save=$(html_str);
j_save.find(".save_list").css("font-family",that.kag.config.userFace);
j_save.find(".save_display_area").each(function(){
$(this).click(function(e){var num=$(this).attr("data-num");
...
if($.userenv()!="pc"){j_save.find(".button_smart").show();
j_save.find(".button_arrow_up").click(function(){var now=j_save.find(".area_save_list").scrollTop();
var pos=now-160;layer_menu.find(".area_save_list").animate({scrollTop:pos},{queue:false})});
j_save.find(".button_arrow_down").click(function(){var now=j_save.find(".area_save_list").scrollTop();
var pos=now+160;
...
var layer_menu=that.kag.layer.getMenuLayer();
for(var i=0;i<array.length;i++)array[i].num=i;
this.kag.html("load_2",{array_save:array,"novel":$.novel},
function(html_str){var j_save=$(html_str);
j_save.find(".save_list").css("font-family",that.kag.config.userFace);
j_save.find(".save_display_area").each(function(){
$(this).click(function(e){var num=$(this).attr("data-num");
...
loadGame:function(num){var array_save=this.getSaveData();var array=array_save.data;if(array[num].save_date=="")return;var auto_next="no";if(array[num].stat.load_auto_next==true)auto_next="yes";
...
if(typeof Live2Dcanvas!="undefined")for(model_id in Live2Dcanvas)if(Live2Dcanvas[model_id]){Live2Dcanvas[model_id].check_delete=2;
Live2D.deleteBuffer(Live2Dcanvas[model_id].modelno);
delete Live2Dcanvas[model_id]}this.kag.layer.setLayerHtml(data.layer);
...
if(options.bgm_over=="false"){var map_se=this.kag.tmp.map_se;
for(var key in map_se)if(map_se[key])this.kag.ftag.startTag("stopse",{stop:"true",buf:key});
var map_bgm=this.kag.tmp.map_bgm;for(var key in map_bgm)this.kag.ftag.startTag("stopbgm",{stop:"true",buf:key});
if(this.kag.stat.current_bgm!=""){var mstorage=this.kag.stat.current_bgm;
var pm={loop:"true",storage:mstorage,stop:"true"};
if(this.kag.stat.current_bgm_vol!="")pm["volume"]=this.kag.stat.current_bgm_vol;
this.kag.ftag.startTag("playbgm",pm)}for(key in this.kag.stat.current_se){var pm_obj=this.kag.stat.current_se[key];pm_obj["stop"]="true";
this.kag.ftag.startTag("playse",pm_obj)}}if(this.kag.stat.cssload)for(file in this.kag.stat.cssload){var style='<link rel="stylesheet" href="'+file+"?"+Math.floor(Math.random()*1E7)+'">';$("head link:last").after(style)}else this.kag.stat.cssload={};
if(!this.kag.stat.current_bgmovie)this.kag.stat.current_bgmovie={storage:"",volume:""};
if(this.kag.config.useCamera==
...
var a3d_define={frames:{"0%":{trans:this.kag.stat.current_camera[key]},"100%":{trans:this.kag.stat.current_camera[key]}},config:{duration:"5ms",state:"running",easing:"ease"},
complete:function(){}};if(key=="layer_camera"){$(".layer_camera").css("-webkit-transform-origin",
...
false;this.kag.ftag.startTag("bgmovie",pm)}this.kag.setCursor(this.kag.stat.current_cursor);if(this.kag.stat.visible_menu_button==true)$(".button_menu").show();else $(".button_menu").hide();$(".event-setting-element").each(function(){var j_elm=$(this);var kind=j_elm.attr("data-event-tag");var pm=JSON.parse(j_elm.attr("data-event-pm"));var event_tag=object(tyrano.plugin.kag.tag[kind]);event_tag.setEvent(j_elm,pm)});
...
loadGameData_2:function(data,options){
var auto_next="no";
if(typeof options=="undefined")options=
{bgm_over:"false"};
else if(typeof options.bgm_over=="undefined")
...
if(this.kag.stat.cssload)for(file in this.kag.stat.cssload){var style='<link rel="stylesheet" href="'+file+"?"+Math.floor(Math.random()*1E7)+'">';$("head link:last").after(style)}else this.kag.stat.cssload={};
if(!this.kag.stat.current_bgmovie)this.kag.stat.current_bgmovie={storage:"",volume:""};
if(this.kag.config.useCamera==
"true"){$(".layer_camera").css({"-animation-name":"","-animation-duration":"","-animation-play-state":"","-animation-delay":"","-animation-iteration-count":"","-animation-direction":"","-animation-fill-mode":"","-animation-timing-function":""});
for(key in this.kag.stat.current_camera){
...
"center center");setTimeout(function(){$(".layer_camera").a3d(a3d_define)},1)}else{$("."+key+"_fore").css("-webkit-transform-origin","center center");setTimeout(function(){$("."+key+"_fore").a3d(a3d_define)},1)}}}$(".tyrano_base").find("video").remove();this.kag.tmp.video_playing=false;if(this.kag.stat.current_bgmovie["storage"]!=""){var vstorage=this.kag.stat.current_bgmovie["storage"];var volume=this.kag.stat.current_bgmovie["volume"];var pm={storage:vstorage,volume:volume,stop:"true"};this.kag.tmp.video_playing=
false;this.kag.ftag.startTag("bgmovie",pm)}this.kag.setCursor(this.kag.stat.current_cursor);if(this.kag.stat.visible_menu_button==true)$(".button_menu").show();else $(".button_menu").hide();$(".event-setting-element").each(function(){var j_elm=$(this);var kind=j_elm.attr("data-event-tag");var pm=JSON.parse(j_elm.attr("data-event-pm"));var event_tag=object(tyrano.plugin.kag.tag[kind]);event_tag.setEvent(j_elm,pm)});
var insert={name:"call",pm:{storage:"make_2.ks","auto_next":auto_next},val:""};this.kag.clearTmpVariable();
this.kag.ftag.nextOrderWithIndex(data.current_order_index,data.stat.current_scenario,true,insert,"yes")},
...
setMenu:function(j_obj,cb){var that=this;var layer_menu=this.kag.layer.getMenuLayer();
j_obj.find(".menu_close").click(function(e){layer_menu.fadeOut(300,function(){tyrano.plugin.kag.variable.sf.back=2;that.kag.menu.loadQuickSave();layer_menu.empty();
if(typeof cb=="function")cb()});if(that.kag.stat.visible_menu_button==true)$(".button_menu").show()});
j_obj.hide();
j_obj.find(".menu_close_save").click(function(e){layer_menu.fadeOut(300,function(){layer_menu.empty();
if(typeof cb=="function")cb()});if(that.kag.stat.visible_menu_button==true)$(".button_menu").show()});

j_obj.find(".menu_close_load").click(function(e){layer_menu.fadeOut(300,function(){layer_menu.empty();
if(typeof cb=="function")cb()});if(that.kag.stat.visible_menu_button==true)$(".button_menu").show()});


j_obj.hide();
layer_menu.append(j_obj);layer_menu.show();$.preloadImgCallback(layer_menu,function(){j_obj.fadeIn(300);
layer_menu.find(".block_menu").fadeOut(300)},that)},hideMenu:function(){},
getSaveData:
function(){var tmp_array=$.getStorage(this.kag.config.projectID+"_tyrano_data",this.kag.config.configSave);if(tmp_array)return JSON.parse(tmp_array);else{tmp_array=new Array;var root={kind:"save"};var save_slot_num=this.kag.config.configSaveSlotNum||5;for(var i=0;i<save_slot_num;i++){var json={};json.title=$.lang("not_saved");json.current_order_index=0;json.save_date="";json.img_data="";json.stat={};tmp_array.push(json)}root.data=
tmp_array;return root}},
displayLog:function(){var that=this;this.kag.stat.is_skip=false;var j_save=$("<div></div>");this.kag.html("backlog",{"novel":$.novel},function(html_str){var j_menu=$(html_str);var layer_menu=that.kag.layer.getMenuLayer();layer_menu.empty();layer_menu.append(j_menu);
layer_menu.find(".menu_close").click(function(){tyrano.plugin.kag.variable.sf.back=2;that.kag.menu.loadQuickSave();layer_menu.hide();
layer_menu.find(".button_smart").hide();
if(that.kag.stat.visible_menu_button==true)$(".button_menu").show()});
layer_menu.find(".menu_close_save").click(function(){layer_menu.hide();
if(that.kag.stat.visible_menu_button==true)$(".button_menu").show()});

layer_menu.find(".menu_close_load").click(function(){layer_menu.hide();
if(that.kag.stat.visible_menu_button==true)$(".button_menu").show()});


layer_menu.find(".button_smart").hide();
layer_menu.find(".menu_close_log").click(function(){layer_menu.hide();
if(that.kag.stat.visible_menu_button==true)$(".button_menu").show()});
layer_menu.find(".button_smart").hide();
